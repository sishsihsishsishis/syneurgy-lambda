...
async function createLicense(db, user_email, subscription_id, status, account_created, stripe_data, is_admin=false, free_account=false, price) {
  subscription_id = free_account == true ? null : subscription_id;
  is_admin = free_account == true ? false : is_admin;
  return db.query("INSERT INTO licenses (id, user_email, subscription_id, status, account_created, stripe_data, is_admin, free_account, created_date, price) VALUES (nextval('licenses_id_seq'::regclass), $1, $2, $3, $4, $5, $6, $7, NOW(), $8) RETURNING *", [user_email, subscription_id, status, account_created, stripe_data, is_admin, free_account, price]);
}
